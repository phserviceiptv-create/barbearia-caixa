/* Controle de acesso Free x PRO.
   FREE: Caixa + Agenda + edição dos dados da barbearia.
...
(function(){
  ...
  function isPro(){ return pro || String(window.empresa?.plano_acesso||'').toLowerCase()==='pro'; }
  ...
  async function checkPlan(force=false){
    ...
    lastPlanCheck=now;
    try{
      const r=await sb.from('assinaturas').select('status,ultima_confirmacao_em,asaas_payment_id').eq('empresa_id',window.empresa.id).order('created_at',{ascending:false}).limit(1).maybeSingle();
      const a=r.data;
      pro=String(window.empresa?.plano_acesso||'').toLowerCase()==='pro' && a?.status==='ativa' && !!a?.ultima_confirmacao_em && !!a?.asaas_payment_id;
      if(!pro && String(window.empresa?.plano_acesso||'').toLowerCase()==='pro'){
        await sb.from('empresas').update({plano_acesso:'free'}).eq('id',window.empresa.id);
        window.empresa.plano_acesso='free';
      }
    }catch(e){ pro=false; }
    apply();
    return pro;
  ...
  function toast(){
    const old=$('accessToast'); if(old) old.remove();
    const el=document.createElement('div'); el.id='accessToast';
  ...
  function lockTab(btn){
    ...
    btn.dataset.proLocked='1'; btn.classList.add('pro-locked');
    const original=btn.textContent.trim(); btn.dataset.originalText=original;
    btn.innerHTML='<span>'+original+'</span><span class="pro-lock">🔒 PRO</span>'; btn.addEventListener('click',gateClick,true);
  }
  function unlockTab(btn){
    if(btn.dataset.proLocked!=='1') return;
    btn.dataset.proLocked='0'; btn.classList.remove('pro-locked'); btn.innerHTML=btn.dataset.originalText||btn.textContent.replace('🔒 PRO','').trim();
  }
  ...
  function lockSettingsSection(card){
    if(!card) return; card.classList.add('pro-section-locked');
    if(card.querySelector('.pro-section-overlay')) return;
    const overlay=document.createElement('div'); overlay.className='pro-section-overlay';
    ...
    overlay.querySelector('button').onclick=()=>showUpgrade(); card.appendChild(overlay);
  }
  function unlockSettingsSection(card){ if(!card) return; card.classList.remove('pro-section-locked'); card.querySelector('.pro-section-overlay')?.remove(); }

  function applySettingsAccess(){
    const settings=$('configuracoes'); if(!settings) return;
    const cards=settings.querySelectorAll('.settings-card'); if(cards.length<3) return;
    const servicesCard=cards[1], hoursCard=cards[2];
    if(isPro()){ unlockSettingsSection(servicesCard); unlockSettingsSection(hoursCard); }
    else { lockSettingsSection(servicesCard); lockSettingsSection(hoursCard); }
  }
  ...
  function applyPaymentOptions(){
    const sel=$('paymentSelect'); if(!sel) return;
    let card=sel.querySelector('option[value="CREDIT_CARD"]');
    const old=sel.querySelector('option[value="UNDEFINED"]');
    if(!card){ card=document.createElement('option'); card.value='CREDIT_CARD'; card.textContent='Cartão de crédito'; }
    if(old) old.replaceWith(card); else if(!sel.querySelector('option[value="CREDIT_CARD"]')) sel.appendChild(card);
    if(sel.value==='UNDEFINED') sel.value='PIX';
  }

  function apply(){
    addStyles(); applyPaymentOptions();
    ...
    if(!isPro()){
      const app=$('appView'); if(app){document.querySelectorAll('.panel').forEach(p=>{if(p.id!=='dashboard'&&p.id!=='agenda'&&p.id!=='configuracoes')p.classList.remove('active');});$('dashboard')?.classList.add('active');}
    }
    const planTrigger=$('planTrigger'); if(planTrigger&&!isPro()){const label=planTrigger.querySelector('.plan-label');if(label)label.textContent='Plano Gratuito';}
    applySettingsAccess();
  }

  function observe(){
    apply(); checkPlan(true); setInterval(()=>checkPlan(false),5000);
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)checkPlan(true);});
    const mo=new MutationObserver(()=>applyPaymentOptions()); mo.observe(document.body,{childList:true,subtree:true});
  ...
  const boot=setInterval(()=>{if($('appView')&&!$('appView').classList.contains('hidden')&&window.empresa?.id){clearInterval(boot);observe();}},300);
})();
