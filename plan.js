(function(){
  ...
  function openPlan(){
    ...
    overlay.innerHTML=`<div class="plan-box" role="dialog" aria-modal="true"><div class="plan-head"><div><h2>${pro?'Seu plano PRO':'Escolha como usar o Barbearia Caixa'}</h2><p class="plan-sub">A versão gratuita continua disponível. O PRO fica dentro da mesma aplicação.</p></div><button class="plan-close" id="planClose" aria-label="Fechar">×</button></div><div class="plan-grid"><div class="plan-card"><h3>Gratuito</h3><div class="plan-price">R$ 0</div><ul><li>Caixa e movimentações</li><li>Clientes</li><li>Agenda</li><li>Histórico</li><li>Configurações básicas</li></ul><button class="plan-secondary" id="planFreeBtn" type="button" disabled>Plano atual</button></div><div class="plan-card pro-card"><h3>PRO</h3><div class="plan-price">a partir de R$ 29,90</div><ul><li>Recursos avançados do sistema</li><li>Relatórios e gestão ampliada</li><li>Recibos e ferramentas profissionais</li><li>Atualizações e recursos PRO</li><li>Suporte prioritário</li></ul>${pro?'<button class="plan-secondary" type="button" disabled>PRO ativo</button>':`<label style="display:block;font-weight:700">Escolha a cobrança</label><select id="planSelect" class="plan-select"><option value="mensal">Mensal — ${money(29.90)}</option><option value="trimestral">Trimestral — ${money(79.90)}</option><option value="semestral">Semestral — ${money(149.90)}</option><option value="anual">Anual — ${money(249.90)}</option></select><button id="planUpgradeBtn" class="plan-primary" type="button">Assinar PRO</button>`}</div></div>${status && !pro?`<div class="plan-status warn">Assinatura ${status}. Se você já pagou, clique em <b>Atualizar status</b>.</div>`:''}${pro?`<div class="plan-status ok">Seu acesso PRO está ativo${current?' · '+current.label:''}.</div>`:''}<div style="display:flex;gap:10px;margin-top:14px;justify-content:flex-end"><button class="plan-secondary" id="planRefresh" type="button">Atualizar status</button></div><div id="planMsg" class="plan-note">O pagamento é processado pelo Asaas. A aplicação libera o PRO somente após a confirmação do pagamento.</div></div>`;
