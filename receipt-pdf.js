(function(){
  ...
  function receiptData(a,c){
    ...
    const mov=window.movimentacoes?.find(x=>x.atendimento_id===a?.id);
  ...
  async function buildReceiptPDF(a,c){
    ...
    row('Cliente',d.client,true);row('Serviço',d.service);if(d.professional)row('Profissional',d.professional);row('Data',d.date);row('Hora',d.time);row('Pagamento',d.payment);
    y+=1;line();doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text('VALOR:',5,y);doc.text(money(d.value),width-5,y,{align:'right'});y+=6;line();center('Obrigado pela preferência!',8.5,true);center('Volte sempre!',8.5);
    return doc;
  }
  async function shareReceiptPDF(a,c){
    ...
    const url=URL.createObjectURL(blob),aEl=document.createElement('a');aEl.href=url;aEl.download=name;aEl.click();setTimeout(()=>URL.revokeObjectURL(url),3000);
