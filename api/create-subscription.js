...
module.exports=async(req,res)=>{
 ...
 try{
  ...
  const cr=await fetch(`${ASAAS_URL}/customers`,{method:'POST',headers:ah(key),body:JSON.stringify({name:e.nome_fantasia||e.nome||user.email,email:e.email||user.email,phone:e.telefone||e.whatsapp||undefined,cpfCnpj:documento,externalReference:empresaId})});const cj=await body(cr);if(!cr.ok)return res.status(502).json({error:cj?.errors?.[0]?.description||cj?.message||'Erro ao cadastrar cliente no Asaas.'});
  ...
  let invoiceUrl=sj.invoiceUrl||null;let pix=null;
  for(let attempt=0;attempt<4;attempt++){
    const rr=await fetch(`${ASAAS_URL}/subscriptions/${encodeURIComponent(sj.id)}/payments`,{headers:ah(key)});const rj=await body(rr);const payment=rj?.data?.[0];
    invoiceUrl=invoiceUrl||payment?.invoiceUrl||null;
    if(billingType==='PIX'&&payment?.id){
      const qr=await fetch(`${ASAAS_URL}/payments/${encodeURIComponent(payment.id)}/pixQrCode`,{headers:ah(key)});const qj=await body(qr);
