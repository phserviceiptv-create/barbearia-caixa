...
module.exports=async(req,res)=>{
 ...
 try{
  ...
  const er=await fetch(`${SUPABASE_URL}/rest/v1/empresas?id=eq.${encodeURIComponent(empresaId)}&select=nome,nome_fantasia,whatsapp,telefone,email,cnpj,cpf_cnpj`,{headers:h(token)});const es=await body(er);const e=es?.[0]||{};
  const cr=await fetch(`${ASAAS_URL}/customers`,{method:'POST',headers:ah(key),body:JSON.stringify({name:e.nome_fantasia||e.nome||user.email,email:e.email||user.email,phone:e.telefone||e.whatsapp||undefined,cpfCnpj:e.cpf_cnpj||e.cnpj||undefined,externalReference:empresaId})});const cj=await body(cr);if(!cr.ok)return res.status(502).json({error:cj?.errors?.[0]?.description||cj?.message||'Erro ao cadastrar cliente no Asaas.'});
  const base=`https://${req.headers.host}`;const sr=await fetch(`${ASAAS_URL}/subscriptions`,{method:'POST',headers:ah(key),body:JSON.stringify({customer:cj.id,billingType:'UNDEFINED',value:plan.valor,nextDueDate:new Date().toISOString().slice(0,10),cycle:plan.cycle,description:`Barbearia Caixa PRO - ${plan.label}`,externalReference:`${empresaId}:${user.id}:${req.body.plan}`,callback:{successUrl:`${base}/?modo=pago&pagamento=retorno`,autoRedirect:true}})});const sj=await body(sr);if(!sr.ok)return res.status(502).json({error:sj?.errors?.[0]?.description||sj?.message||'Erro ao criar assinatura no Asaas.'});
  ...
  const ir=await fetch(`${SUPABASE_URL}/rest/v1/assinaturas`,{method:'POST',headers:{...h(token),Prefer:'return=representation'},body:JSON.stringify(row)});const ij=await body(ir);if(!ir.ok)return res.status(502).json({error:'Assinatura criada no Asaas, mas não foi registrada no aplicativo.',detail:ij});
  let invoiceUrl=sj.invoiceUrl||null;if(!invoiceUrl){const rr=await fetch(`${ASAAS_URL}/subscriptions/${encodeURIComponent(sj.id)}/payments`,{headers:ah(key)});const rj=await body(rr);invoiceUrl=rj?.data?.[0]?.invoiceUrl||null;}
  return res.status(200).json({checkout_url:invoiceUrl,assinatura:ij?.[0]||null});
